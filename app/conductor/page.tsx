...
export default function ConductorPage() {
	...
	useEffect(() => {
		...
		const syncCounts = async () => {
			try {
				const state = await getRealtimeState(sessionId);
				if (!cancelled) {
					setConnectedUsers({
						left: state.userCount.left,
						right: state.userCount.right,
					});
				}
			} catch (error) {
				console.error("Realtime count sync failed:", error);
			}
		};

		syncCounts();
		const interval = window.setInterval(syncCounts, 1000);
		...
		try {
			...
			if (videoRef.current) {
				...
				postRealtimeMessage({
					...
					console.error("Realtime conductor start failed:", error);
				});

				if (handsRef.current) {
					const { Camera: MediaPipeCamera } = await import(
	...
	const stopCamera = () => {
		...
		if (sessionId) {
			postRealtimeMessage({
				...
				console.error("Realtime conductor stop failed:", error);
			});
		}
	};
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center space-x-4">
							<div className="flex items-center space-x-2 px-4 py-2 bg-zinc-800 rounded-md">
								<div
									className={`w-2 h-2 rounded-full ${isActive ? "bg-green-500" : "bg-zinc-600"}`}
								/>
					...
					<div className="lg:col-span-2 space-y-6">
						...
						<div className="bg-zinc-900 rounded-lg p-6 border border-zinc-800">
							...
							<div className="mt-4 grid grid-cols-2 gap-3">
								<div
									className={`px-4 py-2.5 rounded-md border ${handsDetected.left ? "bg-green-950 border-green-800" : "bg-zinc-800 border-zinc-700"}`}
								>
									<div className="flex items-center justify-between">
										<span className="text-sm text-white font-medium">LEFT</span>
										<div
											className={`w-2 h-2 rounded-full ${handsDetected.left ? "bg-green-500" : "bg-zinc-600"}`}
										/>
									</div>
								</div>
								<div
									className={`px-4 py-2.5 rounded-md border ${handsDetected.right ? "bg-green-950 border-green-800" : "bg-zinc-800 border-zinc-700"}`}
								>
									<div className="flex items-center justify-between">
										<span className="text-sm text-white font-medium">
											RIGHT
										...
										<div
											className={`w-2 h-2 rounded-full ${handsDetected.right ? "bg-green-500" : "bg-zinc-600"}`}
										/>
									</div>
								</div>
							</div>
